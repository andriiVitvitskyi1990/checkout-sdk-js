...
export default class PaymentRequestTransformer {
    ...
    transform(payment: Payment, checkoutState: InternalCheckoutSelectors): PaymentRequestBody {
        ...
        return {
            ...
            paymentMethod: paymentMethod && (paymentMethod.id === 'braintreevenmo' ? {...this._transformPaymentMethod(paymentMethod), id: 'braintreepaypal'} : {...this._transformPaymentMethod(paymentMethod), id: paymentMethod.id}),
            quoteMeta: {
                request: {
                    ...paymentMeta,
